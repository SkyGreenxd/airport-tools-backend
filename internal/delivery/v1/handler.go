...
//	@Description	Возвращает список транзакций QA.<br> Можно фильтровать по статусу с помощью query-параметра `status`.<br> Допустимые значения: <br> - 'qa' вернёт только транзакции, требующие проверки QA;<br> - `closed` вернет закрытые транзакции;<br> - `open` вернет открытые транзакции;<br> - `failed` вернет транзакции с неудачной выдачей инструментов.<br> Каждая транзакция содержит минимальные данные: ID, инженера, номер набора инструментов, дату создания транзакции, текущий статус.
//
//	@Tags			QA
//	@Accept			json
//	@Produce		json
