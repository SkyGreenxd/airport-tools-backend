...
//	@Description	Возвращает список транзакций QA.<br> Можно фильтровать по статусу с помощью query-параметра `status`.<br> Например, `?status=manual_check_required` вернёт только транзакции, требующие ручной проверки.<br> Каждая транзакция содержит минимальные данные: ID, инженера, дату создания, текущий статус.
//
//	@Tags			transactions
//	@Accept			json
//	@Produce		json
